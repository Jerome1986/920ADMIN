...
const open = (row: Partial<ProductFormModel>) => {
  ...
  // 打开弹窗时自动获取二级分类
  tocCateStore.subCateListTocGet(1, 100, props.parentId)
  ...
  if (row._id) {
    ...
    if (props.parentId) resetFormModel(props.parentId) // 重置表单
  ...
  skuFileList.value = formModel.value.skus?.flatMap((sku) =>
    sku.image ? [{ name: 'cover', url: sku.image }] : []
  )
  console.log(skuFileList.value)
}
  ...
  <div class="proChannel">
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="40%">
      ...
      >
        ...
        <el-form-item label="适用型号" prop="models">
          <el-select
            ...
            style="width: 100%"
          >
            <el-option v-for="item in PHONE_MODELS" :key="item" :label="item" :value="item" />
          </el-select>
        </el-form-item>
        <!-- 热门推荐  -->
        ...
        <el-row :gutter="12">
          <el-col :span="12">
            ...
            <el-form-item
              label="二级分类"
              prop="subCategoryId"
              v-if="tocCateStore.subCateTocList.length > 0"
            >
              <el-select
              ...
              >
                <el-option
                  v-for="item in tocCateStore.subCateTocList"
                  :key="item._id"
                  :label="item.name"
                  :value="item._id"
                />

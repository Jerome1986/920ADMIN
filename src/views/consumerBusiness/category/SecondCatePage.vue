...
import type { JelConsumerSubCateChannel } from '@/types/Components'
...
// 获取子组件
const dialog = ref<JelConsumerSubCateChannel>()
...
const handleSizeChange = (size: number) => {
  ...
  cateTocStore.subCateListTocGet(params.value.pageNum, params.value.pageSize, categoryId.value) // 子级分类
}
...
const onDelChannel = (row: CateItem) => {
  ...
  ElMessageBox.confirm('你确定要删除吗？', '提示', {
    ...
    .then(async () => {
      const delRes = await cateTocDeleteApi(row._id)
      console.log('onDelChannel', row, delRes)
      await cateTocStore.subCateListTocGet(
...
onMounted(async () => {
  await cateTocStore.cateTocListGet(1, 100) // 一级分类
  categoryId.value = cateTocStore.cateTocList[0]?._id ?? '' // 父级分类ID
  await cateTocStore.subCateListTocGet(
    params.value.pageNum,
    params.value.pageSize,
    categoryId.value
  ...
  <PageContainer title="分类管理">
    <div class="header">
      <el-tabs v-model="categoryId" class="tabs" @tab-click="handleClick">
        <el-tab-pane
          v-for="item in cateTocStore.cateTocList"
          :key="item._id"
          :label="item.name"
          :name="item._id"
        ></el-tab-pane>
    ...
    <!--  对应一级分类的二级分类  -->
    <el-table
      :data="cateTocStore.subCateTocList"
      style="width: 100%"
    ...
    >
      ...
      <el-table-column label="所属分类" align="center" prop="parentId">
        <template #default="{ row }">
          {{ cateTocStore.mapCateParentName(row.parentId) }}
        </template>
      </el-table-column>

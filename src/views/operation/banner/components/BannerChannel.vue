...
import type { BannerFormModel, BannerItem } from '@/types/BannerItem.d.ts'
import { Plus } from '@element-plus/icons-vue'
import { bannerListAddApi, bannerListUpdateApi } from '@/api/banner.ts'
...
// 弹窗表单
const formModel = ref<BannerFormModel>({
  _id: '',
  url: ''
})
...
// 打开弹窗
const open = (row: Partial<BannerItem> = {}) => {
  ...
  // 回显
  coverFileList.value = formModel.value.url ? [{ name: 'cover', url: formModel.value.url }] : []
}
...
const onSubmit = async () => {
  ...
  if (formModel.value._id) {
    try {
      const upRes = await bannerListUpdateApi(formModel.value._id, formModel.value.url)
    ...
    try {
      //  新增
      const addRes = await bannerListAddApi(formModel.value.url)

...
import { cateDeleteApi, cateLevelGetApi } from '@/api/cate.ts'
...
const cateListLevelGet = async (level: number) => {
  const res = await cateLevelGetApi(level)
  cateList.value = res.data
}

// 映射父级分类名称
const mapParentCateName = (parentId: string) => {
  if (!parentId) return ''
  const newCate = cateList.value.find((item) => item._id === parentId)
  return newCate?.name
}
...
const handleSizeChange = (size: number) => {
  ...
  cateStore.thirdCateListGet(params.value.pageNum, params.value.pageSize, categoryId.value)
}
...
const onDelChannel = (row: CateItem) => {
  ...
  ElMessageBox.confirm('你确定要删除吗？', '提示', {
    ...
    .then(async () => {
      ...
      await cateStore.thirdCateListGet(
...
onMounted(async () => {
  await cateListLevelGet(2)
  categoryId.value = cateList.value[0]?._id ?? ''
  await cateStore.thirdCateListGet(params.value.pageNum, params.value.pageSize, categoryId.value)
})
</script>
...
<template>
  <PageContainer title="分类管理">
    ...
    <!--  对应一级分类的二级分类  -->
    <el-table :data="cateStore.thirdCateList" style="width: 100%" v-loading="cateStore.loading">
      ...
      <el-table-column label="所属分类" align="center" prop="parentId">
        <template #default="{ row }">
          {{ mapParentCateName(row.parentId) }}
        </template>
      </el-table-column>

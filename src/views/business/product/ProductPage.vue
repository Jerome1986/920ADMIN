...
import type { JelProductChannel } from '@/types/Components'
import ProductChannel from './components/ProductChannel.vue'
import { productDeleteApi, productGetApi, searchProductApi } from '@/api/product.ts'
...
// 组件
const dialog = ref<JelProductChannel>()
...
const handleClearSearch = () => {
  ...
  if (activeName.value)
    productListGet(activeName.value, params.value.pageNum, params.value.pageSize)
}
...
const productListGet = async (cateId: string, pageNum: number, pageSize: number) => {
  loading.value = true
  const res = await productGetApi('toB', cateId, pageNum, pageSize)
...
const handleSizeChange = (size: number) => {
  ...
  if (searchValue.value) {
    searchMaterialGet(params.value.pageNum, params.value.pageSize)
    return
  }
  // 重新获取数据
  productListGet(activeName.value!, params.value.pageNum, params.value.pageSize)
...
const onDelChannel = (row: ProductItem) => {
  ...
  ElMessageBox.confirm('你确定要删除吗？', '提示', {
    ...
    .then(async () => {
      // 删除逻辑
      if (row._id && activeName.value) {
        const delRes = await productDeleteApi('toB', row._id)
        console.log('删除结果', delRes)
        // 重新获取数据
        await productListGet(activeName.value, params.value.pageNum, params.value.pageSize)
...
// 当前激活的一级分类
const activeName = ref(cateStore.cateList[0]?.name)
// 标签点击选择
...
onMounted(async () => {
  await cateStore.cateListGet(1, 100)
  if (cateStore.cateList.length > 0) {
    activeName.value = cateStore.cateList[0]!._id // TAB默认激活第一项
    await productListGet(activeName.value, params.value.pageNum, params.value.pageSize)
  }
})
</script>
...
<template>
  ...
  <!-- 弹窗 -->
  <ProductChannel ref="dialog" @success="handleSuccess" :parent-id="activeName"></ProductChannel>
</template>
